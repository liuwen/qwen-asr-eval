...
12. Writes JSONL, transcript Markdown/text, and a placeholder judge JSON into Drive under `runs/<run_id>/outputs/`.
13. Leaves Colab GenAI text-only triage to the notebook/Web UI. CLI scripts intentionally do not call `google.colab.ai`.
...
## Colab GenAI Web UI requirement

Do not call `google.colab.ai` from `colab exec` scripts and do not try to supply `MODEL_PROXY_API_KEY` or `GEMINI_API_KEY`. Colab GenAI is a Web UI capability for this workflow. Run ASR with scripts or notebooks, then run text-only triage/proofreading/enhancement cells manually in the Colab Web UI.
